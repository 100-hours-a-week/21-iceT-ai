...
SOLUTION_PROMPT = PromptTemplate(
    ...
    **절대** Markdown 문법(백틱```, *별표*, 1. 숫자 리스트, **굵게** 등)을 사용하지 말고, 모든 필드는 **순수 JSON 텍스트**로만 작성해야 합니다.
    특히 알고리즘 설명 부분은 **반드시** '참고 문서' 내용을 직접 참조하여 정의, 작동 방법, 시간복잡도를 포함한 내용을 줄 글로 자연스럽게 이어서 상세하게 설명해야 합니다.
    ...
    {{
        ...
        "problem_check": {{
            ...
            "algorithm": str             // 알고리즘 이름, 정의, 작동 방식, 시간복잡도가 포함된 내용을 줄 글로 자연스럽게 이어서 설명 (참고 문서에 해당 알고리즘이 존재하는 경우 반드시 참고 문서에 기반하여 기술)
        }},
    ...
    - 'algorithm': 알고리즘 이름, 정의, 작동 방식, 시간복잡도가 포함된 내용을 줄 글로 자연스럽게 이어서 설명 (참고 문서에 없는 경우는 직접 설명)
